...
MODAL_EDGE_DROPOUT = 0.3  
MODAL_MASK_RATIO = 0.2  
...
class MultimodalFeatureExtractor:
    def __init__(self, num_users, num_items, movies_df, users_df):
        ...
        print(f"多模态特征维度:")
        print(f"  视觉特征: {self.visual_features.shape}")
...
class MMGCLDataset:
    ...
    def _build_modal_adj(self):
        ...
        for m in range(NUM_MODALITIES):
            ...
            norm_adj = d_mat_inv_sqrt.dot(adj_mat).dot(d_mat_inv_sqrt).tocsr()
            
            self.modal_adjs.append(norm_adj)
...
class MMGCL(nn.Module):
    def __init__(self, n_users, n_items, emb_dim, modal_adjs, modal_features, n_layers=2):
        super().__init__()
        ...
        self.modal_adjs = modal_adjs
        self._build_sparse_tensors()
        self.to(DEVICE)

    def _build_sparse_tensors(self):
        self.sparse_adjs = []
        for adj in self.modal_adjs:
            adj_coo = adj.tocoo()
            values = adj_coo.data
            ...
            sparse_adj = torch.sparse_coo_tensor(i, v, shape, dtype=torch.float32).coalesce()
            self.sparse_adjs.append(sparse_adj)

    def _modal_edge_dropout(self, adj, dropout_rate):
        adj_coo = adj.tocoo()
    ...
    def _modal_masking(self, modal_emb, mask_ratio):
        mask = torch.rand_like(modal_emb) > mask_ratio
        return modal_emb * mask.float()

    def forward(self, adj=None, mask_modal=False, return_all_modal=False):
        ...
        for m, encoder in enumerate(self.modal_encoders):
            ...
            if mask_modal:
                item_modal = self._modal_masking(item_modal, MODAL_MASK_RATIO)
        ...
        for layer in range(self.n_layers):
            if adj is not None:
                ...
                all_emb = torch.sparse.mm(self.sparse_adjs[0], all_emb)
    ...
    def forward_multi_view(self):
        ...
        modal_idx = np.random.randint(0, self.n_modalities)
        adj_aug = self._modal_edge_dropout(self.modal_adjs[modal_idx], MODAL_EDGE_DROPOUT)
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 3 == 0 or epoch == epochs - 1:
                ...
                if not math.isinf(val_loss) and val_loss < best_val:
                    best_val = val_loss
                    torch.save(self.model.state_dict(), 'mmgcl_best.pt')
                    patience_counter = 0
                else:
                    patience_counter += 1
